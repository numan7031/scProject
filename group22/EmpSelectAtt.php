...
<div class="wrapper row3">
  <main class="hoc container clear">
	...
	echo "<tr>
	<center>
			<th>หมายเลขสถานที่ท่องเที่ยว</th>
			<th>ชื่อสถานที่ท่องเที่ยว</th>
			<th>ที่อยู่</th>
			<th>Latitude</th>
			<th>Longitude</th>
			<th>จุจุดเด่นของสถานที่</th>
			<th>ประเภทของจุดเด่น</th>
			<th>รูปภาพ</th>
			<th>แก้ไขสถานะ</th>
			<th>ลบข้อมูล</th>
	...
	while($row = $rs->fetch_assoc()) {
		...
		echo '<td>' . $row['lat'] . '</td>';
		echo '<td>' . $row['lng'] . '</td>';
		echo '<td>' . $row['typeAttraction'] . '</td>';
		echo '<td>' . $row['type_id'] . '</td>';

  	echo '<td><img src="'. $row['image'] .'" height="100" width="100"/></td>';
		echo '<nav class="main-nav">';
		echo '<td><figcaption><a class="btn small" href="EmpEditAttForm.php?id=' . $row['attracID'] . '">Edit</a></figcaption></td> ';
		echo '<td><figcaption><a class="btn small" href="EmpDeletedAtt.php?id=' . $row['attracID'] . '" onclick="return ConfirmDelete(' . $row['attracID'] . ');">Delete</a></figcaption></td>';
		echo '<td><figcaption><a class="btn small" href="pages/html/insertGalaryAtt.php?resid=' . $row['attracID'] . '" target="_blank">แกลอรี่</a></figcaption></td>';
		echo '</nav>';
		echo "</tr>";
	}
